...
// tvout file define:
#define AUTOTB_TVOUT_PC_a "../tv/rtldatafile/rtl.matrix_mult.autotvout_a.dat"
// tvout file define:
#define AUTOTB_TVOUT_PC_b "../tv/rtldatafile/rtl.matrix_mult.autotvout_b.dat"
// tvout file define:
...
extern "C" void matrix_mult_hw_stub_wrapper(volatile void *, volatile void *, volatile void *);
    ...
    string AESL_num;{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<16> > prod_pc_buffer(25);
          int i = 0;
          ...
          if (i > 0) {{
            int i = 0;
            for (int j = 0, e = 25; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_prod)[j*2+0] = prod_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_prod)[j*2+1] = prod_pc_buffer[i].range(15, 8).to_int64();
}}}
        } // end transaction
      } // end file is good
...
// print prod Transactions
{
  ...
  {  __xlx_offset_byte_param_prod = 0*2;
  if (__xlx_apatb_param_prod) {
    for (int j = 0  - 0, e = 25 - 0; j != e; ++j) {
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_prod)[j];

    std::string s(__xlx_tmp_lv.to_string(SC_HEX));
    aesl_fh.write(AUTOTB_TVIN_prod, s.append("\n")); 
      }
  }
}
...
sc_bv<16> __xlx_tmp_lv = ((short*)__xlx_apatb_param_prod)[j];
    ...
    std::string s(__xlx_tmp_lv.to_string(SC_HEX));
    aesl_fh.write(AUTOTB_TVOUT_prod, s.append("\n")); 
      }
  }
}
  tcl_file.set_num(25, &tcl_file.prod_depth);
